...
block="upstream gefyra_user_upstream { server $2; } upstream cluster_upstream { server $3; } map \$http_x_gefyra_bridge_traffic \$gefyra_backend { \\"True\\"  $2; default  $3; }server {listen $1; location \/ { proxy_pass http:\/\/\$gefyra_backend;proxy_pass_request_headers  on;proxy_set_header Host \$host;}}\n#HTTPMARKER"
