...
@click.option(
    "--tls-key",
    help="Path to key file for tls traffic (within the target container) or secret notation 'secret:{namespace}/{name}:{key}'. To address specific ports, use the format <key>@port",
    type=str,
    required=False,
    multiple=True,
)
@click.option(
    "--tls-certificate",
    help="Path to certificate file for tls traffic (within the target container) or secret notation 'secret:{namespace}/{name}:{key}'. To address specific ports, use the format <cert>@port",
    type=str,
    required=False,
    multiple=True,
)
@click.option(
    "--tls-sni",
    help="SNI for tls traffic (within the target container) or secret notation 'secret:{namespace}/{name}:{key}'. To address specific ports, use the format <sni>@port",
    type=str,
    required=False,
    multiple=True,
)
